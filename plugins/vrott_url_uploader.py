...
async def Vrott_capture(bot, update):
    ...
    if "Vrott" in update.text:
        try:
            w = update.text 
            req1 = requests.get("https://useraction.Vrott.com/tokennd").json()
            rgx = re.findall("([0-9]?\w+)", w)[-3:]
            li = { "url":"io.binarynumbers.vrott.ott", "token":"https://gwapi.Vrott.com/content/details/" }
            req2 = requests.get("https://useraction.vrott.com/token/platform_tokens.php?platform_name=web_app").json()["token"]
            headers["X-Access-Token"] = req2
            req3 = requests.get("https://useraction.Vrott.com/token").json()    
            if "movies" in w:
        ...
        await update.reply_text("I can download from Zee5 links only! Use @Dads_links_bot for other links 😇", quote=True)
        return
...
async def zee5_execute(bot, update):
    ...
    try:
        ...
        youtube_dl_url = Vrott_capture.url
